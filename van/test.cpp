...
*/
  ...
  for(int i =0;i<num_platforms;i++){
    for(int j = 0;j<num_devices[i];j++){
      if(count4+2 != totaldev*3-1 || rest == 0){
        ...
        mem[count4+1] = clCreateBuffer(context[i], CL_MEM_READ_WRITE, sizeof(double)*delta, NULL, &status);
        //cout<<"context["<<i<<"] "<<"create mem["<<count4+1<<"] : "<<status<<endl; 
        mem[count4+2] = clCreateBuffer(context[i], CL_MEM_READ_WRITE, sizeof(double)*delta, NULL, &status);
        ...
        //cout<<"context["<<i<<"] "<<"create mem["<<count4<<"] : "<<status<<endl;
        mem[count4+1] = clCreateBuffer(context[i], CL_MEM_READ_WRITE, sizeof(double)*rest, NULL, &status);
        //cout<<"context["<<i<<"] "<<"create mem["<<count4+1<<"] : "<<status<<endl; 
        mem[count4+2] = clCreateBuffer(context[i], CL_MEM_READ_WRITE, sizeof(double)*rest, NULL, &status);
        //cout<<"context["<<i<<"] "<<"create mem["<<count4+2<<"] : "<<status<<endl;
      }
      count4+=3;
    }
  ...
  for(int i = 0;i<totaldev;i++,count4+=3){
    if(count4+2 != totaldev*3-1 || rest == 0){
      status = clEnqueueWriteBuffer(queue[i], mem[count4], CL_TRUE, 0, sizeof(double)*delta, a_[i], 0,NULL,NULL);
      //cout << "commandQueue["<<i<<"]<=mem["<<count4<<"] : "<< status <<endl; 
      status = clEnqueueWriteBuffer(queue[i], mem[count4+1], CL_TRUE, 0, sizeof(double)*delta, b_[i], 0,NULL,NULL);
      //cout << "commandQueue["<<i<<"]<=mem["<<count4+1<<"] : "<< status <<endl;  
      status = clEnqueueWriteBuffer(queue[i], mem[count4+2], CL_TRUE, 0, sizeof(double)*delta, c, 0,NULL,NULL);
      //cout << "commandQueue["<<i<<"]<=mem["<<count4+2<<"] : "<< status <<endl; 
    }
    else if(count4+2 == totaldev*3-1 && rest != 0){
      status = clEnqueueWriteBuffer(queue[i], mem[count4], CL_TRUE, 0, sizeof(double)*rest, a_[i], 0,NULL,NULL);
      //cout << "commandQueue["<<i<<"]<=mem["<<count4<<"] : "<< status <<endl; 
      status = clEnqueueWriteBuffer(queue[i], mem[count4+1], CL_TRUE, 0, sizeof(double)*rest, b_[i], 0,NULL,NULL);
      //cout << "commandQueue["<<i<<"]<=mem["<<count4+1<<"] : "<< status <<endl;  
      status = clEnqueueWriteBuffer(queue[i], mem[count4+2], CL_TRUE, 0, sizeof(double)*rest, c, 0,NULL,NULL);
      //cout << "commandQueue["<<i<<"]<=mem["<<count4+2<<"] : "<< status <<endl;  
    }
  }
  ...
  for(int i=0;i<totaldev;i++){
    if(3*i+2 != totaldev*3-1 || rest == 0){
      status = clEnqueueReadBuffer(queue[i], mem[3*i+2], CL_TRUE, 0, sizeof(double)*delta, c, 0, NULL, NULL);
      ...
      status = clEnqueueReadBuffer(queue[i], mem[3*i+2], CL_TRUE, 0, sizeof(double)*rest, c, 0, NULL, NULL);
